 ...
 *     <li>$n, $nn - Where n/nn are digits referencing an existent capturing groups, inserts
 *       backreference n/nn.
        ...
        if (isType(replacement, "function")) {
            ...
            result = nativ.replace.call(str, search, function () {
                ...
                return nativ.replace.call(String(replacement), replacementToken, function ($0, $1, $2) {
                     ...
                     * - `$01` is equivalent to `$1` if a capturing groups exists, otherwise it's an error.
                     * - `$0` (not followed by 1-9), `$00`, and `$&` are the entire match.
                     * Native behavior, for comparison:
                     * - Backreferences end after 1 or 2 digits. Cannot use backreference to capturing groups 100+.
                     * - `$1` is a literal `$1` if there are no capturing groups.
                     * - `$10` is `$1` followed by a literal `0` if there are less than 10 capturing groups.
                     * - `$01` is equivalent to `$1` if a capturing groups exists, otherwise it's a literal `$01`.
...
/* Named capturing groups; match the opening delimiter only: (?<name>
    ...
    add(/\\(\d+)/,
        function (match, scope) {
            if (!(scope === defaultScope && /^[1-9]/.test(match[1]) && +match[1] <= this.captureNames.length) &&
                    match[1] !== "0") {
                throw new SyntaxError("can't use octal escape or backreference to undefined groups " + match[0]);
            }
            return match[0];
        },
        {scope: "all"});

/* Capturing groups; match the opening parenthesis only.
    ...
    XRegExp.build = function (pattern, subs, flags) {
        ...
        pattern = pattern.source.replace(parts, function ($0, $1, $2, $3, $4) {
            ...
            if (subName) { // Named subpattern
                ...
                if ($1) { // Named subpattern was wrapped in a capturing groups
